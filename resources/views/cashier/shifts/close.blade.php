            ...
            <div class="rounded-3xl border border-[#F4D3B0]/70 bg-white p-6 shadow-[0_20px_60px_-35px_rgba(31,68,76,0.45)]"
                x-data="{
                    ...
                    formatCurrencyNumber(value) {
                        return new Intl.NumberFormat('id-ID').format(Math.abs(Number(value) || 0));
                    },
                    ...
                    formatCurrency(event) {
                        ...
                        this.closingCash = value ? this.formatCurrencyNumber(value) : '';
                    ...
                    get differenceStatus() {
                        ...
                        return this.difference > 0 ? 'Kelebihan' : 'Kekurangan';
                    },
                
                    get differenceDescription() {
                        if (this.difference === 0) {
                            return 'Uang fisik di laci sesuai dengan perhitungan sistem.';
                        }
                
                        if (this.difference > 0) {
                            return 'Uang fisik di laci lebih besar dari uang kas di sistem.';
                        }
                
                        return 'Uang fisik di laci lebih kecil dari uang kas di sistem.';
                    }
                }">
                ...
                <form x-ref="closeForm" method="POST" action="{{ route('cashier.shifts.close', $shift) }}"
                            ...
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm font-black"
                                        :class="difference < 0 ? 'text-[#A92A35]' : 'text-[#1F444C]'"
                                        x-text="differenceStatus">
                                    </p>
                                </div>

                                <p class="text-xl font-black" :class="difference < 0 ? 'text-[#A92A35]' : 'text-[#1F444C]'"
                                    x-text="differenceFormatted">
                                </p>
                            </div>

                            <p class="mt-3 text-sm font-semibold text-[#6B3E12]" x-text="differenceDescription"></p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="closing_note" rows="4" placeholder="Contoh: Shift ditutup normal."
                            class="block w-full rounded-2xl border border-[#F4D3B0] bg-[#F7F6F4] px-4 py-3 text-sm font-medium text-[#2B1A10] shadow-sm transition placeholder:text-[#6B3E12]/45 focus:border-[#F4B044] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[#F4B044]/20">{{ old('closing_note') }}</textarea>
                    ...
                    <div class="flex flex-col-reverse gap-3 border-t border-[#F4D3B0]/70 pt-6">
                        ...
                        <button type="button" @click="openConfirm()"
                            class="inline-flex h-12 items-center justify-center rounded-2xl bg-[#A92A35] px-6 text-sm font-black text-white shadow-lg shadow-[#A92A35]/20 transition hover:-translate-y-0.5 hover:shadow-xl">
                            Tutup Shift
                        </button>
                    </div>
                </form>
                ...
                <template x-teleport="body">
                    <div x-show="confirmClose" x-cloak
                        ...
                        <div @click.outside="confirmClose = false" x-transition:enter="transition ease-out duration-200"
                            ...
                            <div class="space-y-4 p-6">
                                ...
                                <div class="rounded-2xl bg-white p-4 ring-1 ring-[#F4D3B0]">
                                    ...
                                    <p class="mt-2 text-sm font-semibold text-[#6B3E12]" x-text="differenceDescription">
                                    </p>
                                </div>

                                <p class="text-sm font-semibold leading-relaxed text-[#6B3E12]">
                                    Setelah shift ditutup, shift tidak bisa digunakan lagi untuk transaksi.
                                    Laporan shift akan masuk ke antrean print.
                                </p>
