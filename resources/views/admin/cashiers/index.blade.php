...
@section('content')
    <div class="space-y-6" x-data="{
        ...
        openStatusModal(action, name, currentStatus) {
            ...
            this.statusCashierName = name;
    
            if (currentStatus === 'aktif') {
                this.statusTitle = 'Nonaktifkan Kasir?';
                this.statusMessage = 'Kasir ini tidak bisa login dan tidak dapat digunakan untuk transaksi sampai diaktifkan kembali.';
                this.statusButtonText = 'Ya, Nonaktifkan';
                this.statusButtonClass = 'bg-[#A92A35] text-white shadow-[#A92A35]/20 focus:ring-[#A92A35]/20';
            } else {
                this.statusTitle = 'Aktifkan Kasir?';
                this.statusMessage = 'Kasir ini akan dapat login kembali dan digunakan untuk operasional transaksi.';
                ...
                <div class="relative flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                    <div>
                        ...
                        <p class="mt-2 max-w-2xl text-sm font-medium leading-relaxed text-[#F7F6F4]/80">
                            Kelola akun kasir. Admin dapat menambah, mengedit, menonaktifkan, dan reset password kasir.
                        </p>
                    </div>
        ...
        <div class="rounded-3xl border border-[#F4D3B0]/70 bg-white p-5 shadow-[0_20px_60px_-35px_rgba(31,68,76,0.45)]"
            ...
            <form method="GET" action="{{ route('admin.cashiers.index') }}"
                class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(170px,230px)_auto] lg:items-center xl:grid-cols-[minmax(0,1fr)_310px_auto]">
                            ...
                            <tr class="transition hover:bg-[#F7F6F4]/80">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex flex-wrap items-center justify-end gap-2">
                                        ...
                                        <button type="button"
                                            @click="openStatusModal(@js(route('admin.cashiers.update-status', $cashier)), @js($cashier->name), @js($cashier->status))"
                                            ...
                                            {{ $cashier->status === 'aktif' ? 'Nonaktifkan' : 'Aktifkan' }}
                                        </button>
        ...
        <template x-teleport="body">
            <div x-show="statusModalOpen" x-cloak x-transition.opacity
                ...
                <div @click.outside="closeStatusModal()" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="bg-[#1F444C] px-6 py-5 text-white">
                        <h2 class="text-lg font-black" x-text="statusTitle"></h2>
                        <p class="mt-1 text-sm font-medium text-white/80" x-text="statusMessage"></p>
                    </div>

                    <div class="p-6">
                        <p class="text-sm font-semibold leading-relaxed text-[#6B3E12]">
                            Kasir <span class="font-black text-[#2B1A10]" x-text="statusCashierName"></span>.
                        </p>
